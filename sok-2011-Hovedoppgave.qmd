...
*Nivå på kapitalintensivitet i likevekt(steady-state):* I steady-state har vi altså at: ${\frac{\partial k(t)}{\partial t}}=s \cdot y(t)-n \cdot k(t) = 0$
...
${\frac {s\cdot k(t)^{\alpha-1}}{k(t)^{\alpha-1}}}={\frac{n}{k(t)^{\alpha-1}}}$ Dette er det samme som: 
$s=n\cdot k(t)^{1-\alpha}$ Deler på $n$: ${\frac {s}{n}}={k(t)^{1-\alpha}}$ 
Ganger eksponenten på begge sider med ${\frac{1}{1-\alpha}}$  
Dette gir oss: $k(t)^{ss} = {(\frac {s}{n})}^{\frac{1}{1-\alpha}}$ som er kapitalintensiviteten i langsiktig likevekt(steady state)
...
(11) Kvalitetsindeks til naturressurser: $q_{R(t)}= e^{h\cdot t}$, vekstrate: ${h}$,\
     $0\geq h\geq0$


Vi setter inn uttrykkene for kvalitetsindeksene inn i den spesifikke produktfunksjonen og får:
$Y(t) = A_0\cdot e^{g_A\cdot t}\cdot(e^{j\cdot t}\cdot K(t))^\alpha\cdot(e^{m\cdot t}\cdot L(t))^\beta\cdot(e^{h\cdot t}\cdot R_0\cdot e^{-u\cdot t})^\gamma$  
...
Langs en balansert vekstbane må $g_K-g_Y$ være lik 0 fordi ${\frac {K(t)}{Y(t)}}$ er konstant. Denne delen av uttrykket vil derfor være lik 0. Vi benevner alle vekstratene knyttet til kvalitetsindeksene for $\theta$ og får da at vekstraten i steady state er:
$g_y^{ss}= \theta +({\frac{\beta}{1-\alpha}-1}) \cdot n-{\frac{\gamma}{1-\alpha}}\cdot u$

Vi gjør en liten omregning:
${\frac{\beta}{1-\alpha}-1} = {\frac{\beta}{1-\alpha}-{\frac{1-\alpha}{1-\alpha}}}={\frac{\beta+\alpha-1}{1-\alpha}}$  
En av forutsetningene i modellen er konstant skalutbytte. Dette betyr at summen av produksjonselastisitetene er lik 1, altså at: $\alpha+\beta+\gamma = 1$ som innebærer at vårt uttrykk $\beta+\alpha-1 = -\gamma$

Vi setter dette inn:
$g_y^{ss}= \theta -{\frac{\gamma}{1-\alpha}} \cdot n-{\frac{\gamma}{1-\alpha}}\cdot u$
som da gir:  
...
Brukes ikke:
$Y(t) = A_0\cdot e^{(g_A+\alpha j+\beta m+\gamma h)\cdot t}\cdot K(t)^\alpha \cdot L(t)^\beta \cdot R_0^{\gamma} \cdot e^{-\gamma u\cdot t}$

Brukes ikke:
Vi deler på $L(t)$ for å finne produksjon per arbeider:  
$y(t) = A_0\cdot e^{(g_A+\alpha j+\beta m+\gamma h)\cdot t}\cdot K(t)^\alpha \cdot L(t)^{\beta-1} \cdot R_0^{\gamma} \cdot e^{-\gamma u\cdot t}$

## Metode og data

### Data

Jeg har brukt data fra World Development Indicators som er en stor samling av data som er satt sammen av Verdensbanken. Det er laget en en "package" for R, WDI, for å forenkle analysemulighetene av de tilgjengelige data. Jeg har benyttet meg av denne i datauttaket. Her følger en oversikt over variablene i datasettet:
...
| dppc          | Sjekk med Andrea. Er lik gdppc                                                                                                                                                                                                    |
| ln_gdppc      | logaritmen av gdppc                                                                                                                                                                                                               |
| ln_gdppc0     | logaritmen av gdppc0                                                                                                                                                                                                              |
...
For å se nærmere på relasjonene mellom variablene, setter jeg opp en regresjonsmodell der utdanning er uavhengig variabel og bnp per capita er avhengig variabel.
...
Vi ser av koeffisientene at denne sammenhengen er signifikant fordi p-verdien er veldig lav. Vi kan også lese at i gjennomsnitt vil et år ekstra utdanning øke bnp per capita med 5272.
...
df <- subset(df_growth2019, select = c('ln_gdppc',
                          'avg_gdpgrowth','avg_n','avg_p', 'avg_nsy',
                          'avg_nry','avg_gi','avg_gx','avg_educ'))
...
Avsnitt 4.1 skal også skal inneholde grafer som visualiserer korrelasjoner mellom nivå på og vekst i BNP per innbygger på den ene siden og utvalgte variabler. Lag grafer som viser sammenhengen mellom den naturlige logaritmen av BNP per innbygger på den ene siden og 1) sparing, 2) befolkningsvekst og 3) humankapital på den andre siden. Lag også grafer som visualiserer sammenhengen mellom årlig vekstrate i BNP per innbygger på den ene siden, og 4) sparing og 5) utdanning på den andre siden. Beskriv hva grafene viser.
Legge inn reglinjer?
Lage noe per region
...
```{r}
#| label: fig-saving
#| fig-cap: "Forholdet mellom sparing og BNP per capita"
ggplot(df_growth2019, aes(x= avg_nsy, y = ln_gdppc, color = region) ) +
  geom_point()+
  labs(x= "Gjennomsnittlig sparing for perioden 2000-2015", y = "ln BNP per capita",title= "Forholdet mellom sparing og BNP per capita")
```
**Kommentar**
...
ggplot(df_growth2019, aes(x= avg_p, y = ln_gdppc, color = region) ) +
  geom_point()+
  labs(x= "Gjennomsnittlig årlig vekstrate i befolkningen", y = "log BNP per capita",title= "Forholdet mellom vekstrate i befolkningen og BNP per capita")
```
**Kommentar**

**Sammenhengen mellom utdanningsnivå og BNP per capita**
```{r}
#| label: fig-educ
#| fig-cap: "Forholdet mellom utdanning og BNP per capita"
ggplot(df_growth2019, aes(x= avg_educ, y = ln_gdppc, color=region) ) +
  geom_point()+
  labs(x= "Gjennomsnittlig utdanning i antall år", y = "log BNP per capita",title= "Forholdet mellom utdanning og BNP per capita")
```
...
ggplot(df_growth2019, aes(x= avg_nsy, y = avg_gdpgrowth, color=region) ) +
  geom_point()+
  labs(x= "Gjennomsnittlig sparing for perioden 2000-2015", y = "Vekst i BNP per capita",title= "Forholdet mellom sparing og vekst i BNP per capita")
```
**Kommentar**

**Sammenhengen mellom utdanningsnivå og årlig vekstrate i BNP per capita**
```{r}
#| label: fig-educ_and_growth
...
ggplot(df_growth2019, aes(x= avg_educ, y = avg_gdpgrowth, color=region) ) +
  geom_point()+
  labs(x= "Gjennomsnittlig utdanning i antall år", y = "Vekstrate i BNP per capita",title= "Forholdet mellom utdanning og vekst i BNP per capita")
```
**Kommentar**
...
Fra gjennomgangen av Solow-modellen med teknologi og naturressurser fant vi at vekstraten i BNP per innbygger påvirkes positivt av spareraten, vekstraten i arbeidskraften. Vekstraten påvikres negativt av befolkningsvekst.
...
Lim inn tabellen med regresjonsresultatene i manuskriptet. Her nede har jeg brukt pakken “table1” til å presentere en modell der jeg predikerer nivå på BNP per innbygger ved bruk av gjennomsnittlig antall år i skole. Du finner flere eksempler på hvordan du lager tabeller her: her.
...
- De forklarende variablene er ikke (perfekt) korrelerte med hverandre
- Feiltermene er homoskedastiske
- Det er usannsynlig at det er ekstreme observasjoner (outliers) i utvalget
...
```{r}
model2 <- lm(ln_gdppc  ~ avg_p, data= df_growth2019)
tab_model(model2)
```
**Kommentar**  
...
```{r}
model3 <- lm(ln_gdppc  ~ avg_educ + avg_p, data= df_growth2019)
tab_model(model3)
...
```{r}
model_full <- lm(ln_gdppc  ~ avg_n + avg_p + avg_nsy + avg_nry + avg_gi + avg_educ , data= df_growth2019)
